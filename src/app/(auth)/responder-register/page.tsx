...
import { Ambulance, ArrowRight, ArrowLeft, Eye, EyeOff } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ name, email, password, role: 'responder' }),
      });
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-background">
      ...
      >
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-semibold tracking-tight">Responder registration</h1>
          <p className="text-sm text-muted mt-1">Create your emergency responder account</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="name" className="block text-sm text-foreground font-medium mb-1.5">Full Name</label>
            <input
              id="name"
              value={name}
          ...
          <div>
            ...
            <input
              ...
              placeholder="alex@emergency-service.org"
              className="w-full h-10 px-3.5 rounded-lg bg-surface border border-border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-foreground/20 focus:border-foreground/30 transition-all duration-200"
              required
            />
          </div>
        ...
        <p className="text-center text-sm text-muted mt-6">
          Already registered?{' '}
          <a href="/responder-login" className="text-foreground underline underline-offset-2 hover:opacity-80">
            Sign in
          </a>
        </p>
